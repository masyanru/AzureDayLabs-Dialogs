...
class SecondDialog(CancelAndHelpDialog):
    """Flight booking implementation."""

    def __init__(self, dialog_id: str = None):
        super(SecondDialog, self).__init__(dialog_id or SecondDialog.__name__)
    ...
    async def destination_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        if booking_details.destination is None:
            return await step_context.prompt(TextPrompt.__name__, PromptOptions(
                    prompt=MessageFactory.text("Первый вопрос?")),)
        else:
    ...
    async def origin_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        if booking_details.origin is None:
            return await step_context.prompt(
                TextPrompt.__name__,
                PromptOptions(prompt=MessageFactory.text("Второй вопрос?")),)
        else:
    ...
    async def confirm_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        msg = (
            f"Всё верно? Данные от первого вопроса: { booking_details.destination }"
